...
class Exif:
    ...
    @property
    def comment(self):
        raw_comment = self._gi.get_tag_raw("Exif.Photo.UserComment")
        if raw_comment is None:
            return ""
        size = raw_comment.get_size()
        if size >= 8:
            raw_str = raw_comment.get_data()
            bstr = raw_str[8:]
            encoding = raw_str[:8]
            if encoding.startswith(b"ASCII"):
                encoding = "ASCII"
            elif encoding.startswith(b"Unicode"):
                encoding = "UTF-16"
            elif encoding.startswith(b"JIS"):
                encoding = 'iso-2022-jp'
            else:
                encoding = "UTF-8"
            comment = raw_str.rstrip(b"\x00").decode(encoding)
            return comment
        else:
            return ""
...
#         try:
#             b_comment = comment.encode("ASCII")
#         except:
#             b_comment = b"Unicode\x00" + comment.encode("UTF-16")
#         else:
#             b_comment += "\x00"*3
        self._gi.set_tag_string("Exif.Photo.UserComment", comment)
